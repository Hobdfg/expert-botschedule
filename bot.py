...
def response_start(message):
    ...
    bot.send_message(user_id,
                     ...
                     text=f'Привет, я бот, с которым ты можешь пообщаться на любые темы или '
                          f'задать любые вопросы с помощью текстовых и голосовых сообщений, '
                          f'для работы со мной вы можете использовать одну из команд:\n\n' +
                          commands_to_string(),
                     reply_markup=types.ReplyKeyboardRemove())
